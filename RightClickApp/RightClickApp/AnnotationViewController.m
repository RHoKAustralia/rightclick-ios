...
- (void)viewDidLoad
{
    ...
    // Resize the image to the size of its imageView's content size
    // This is done so that the annotation properly overlayed to the view
    CGRect resizedFrame = AVMakeRectWithAspectRatioInsideRect(self.imageToAnnotate.size,
                                                              self.imageView.bounds);
    NSLog(@"Resizing image to height %f width %f", resizedFrame.size.height, resizedFrame.size.width);
    
    // UIImage *resizedImage = [ImageUtils resizeImageWithImageToResize:self.imageView.image
    //                                                targetImageSize:frame.size];
    //NSLog(@"Resized image to height %f width %f", resizedImage.size.height, resizedImage.size.width);
    ...
    self.jotViewController.drawingContainer.frame = resizedFrame;
}
...
{
    CGRect resizedFrame = AVMakeRectWithAspectRatioInsideRect(self.imageView.image.size,
                                                              self.imageView.bounds);
    NSLog(@"Resizing image to height %f width %f", resizedFrame.size.height, resizedFrame.size.width);
    
    UIImage *drawnImage =  [self.jotViewController drawOnImage:self.imageView.image frame: resizedFrame];
